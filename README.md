...
- `utils_ai.js` - AI服务工具库（Gemini、Groq、OpenRouter API调用）
...
**功能：** 提供多模型AI服务调用功能（Gemini、Groq、OpenRouter），采用命名空间模式封装在UtilsAI对象中
- **设计模式：** 命名空间封装，避免全局作用域污染
...
- **支持平台：** 多AI服务集成 (Gemini, Groq, OpenRouter)

#### 核心功能
- **多模型支持**：集成Gemini、Groq、OpenRouter三大AI服务
